...
function sandbox:enter()
                ...
                gimmickManager:add( LineMarker:new( {
                    ...
                    func = function()
                        ...
                        gimmickManager:add( RectangleAoE:new(
                                                { sx = solider1_x, sy = solider1_y, tx = tx, ty = ty, w = 200, drawPriority = 5, prediction = 0.41 } ), 5 )
                    end,
                    drawPriority = 15
                } ) )
            elseif deb == 'chain2' then
                gimmickManager:add( LineMarker:new( {
                    ...
                    func = function()
                        local tx, ty = select( i, playerManager:getPlayers() ):getPosition()
                        gimmickManager:add( RectangleAoE:new(
                                                { sx = solider2_x, sy = solider2_y, tx = tx, ty = ty, w = 200, drawPriority = 5, prediction = 0.41 } ), 5 )
                    end,
                    drawPriority = 15
                } ) )
    ...
    sequenceManager:add( Sequence:new( function()
        ...
        gimmickManager:add( RectangleAoE:new( { sx = dragon1_x, sy = dragon1_y, tx = x, ty = y, w = 200, drawPriority = 5, prediction = 0.41 } ), 5 )
    end, 15.55 ) )
